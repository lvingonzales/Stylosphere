...
public class advancedLineRenderer : MonoBehaviour
{
    ...
    void Start() {
        ...
        GetComponent<MeshRenderer> ().material = lineMat;
    }
    ...
    public void AddPoint(Vector3 point)
    {
        if(x != Vector3.zero)
        {
             AddLine(ml, MakeQuad(x, point, lineWidth, firstQuad));
             firstQuad = false;
        }
        x = point;
    }

    Vector3[] MakeQuad(Vector3 x, Vector3 e, float w, bool all) 
    {
        w = w / 2;
        ...
        if(all)
        {
            q [0] = transform.InverseTransformPoint (x + l * w);
            q [1] = transform.InverseTransformPoint (x + l * -w);
